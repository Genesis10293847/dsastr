...
export class IncidentReport {
  ...
  addincident() {
    ...
    if (location && date && type && damage && remarks) {
      const incidentCollection = collection(this.firestore, 'disaster_incidents');
      addDoc(incidentCollection, { location, date, type, damage, remarks });
  ...
  updateincident(id: string, newlocation: string, newdate: string, newtype: string, newdamage: string, newremarks: string) {
